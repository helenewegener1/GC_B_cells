...
for (sample_name in sample_names){
  ...
  celltypes_contamination_index <- grep("Contamination", celltypes_unique, value = TRUE)
  
  # Remove DCs and contamination 
  seurat_obj_nonDC <- subset(seurat_obj, !(celltype_broad %in% c("DCs_MNPs", celltypes_contamination_index)))
...
for (sample_name in sample_names){
  ...
  output_dir <- glue("11_ADT_demultiplex/plot/{sample_name}/log10_counts_per_fol")
  ...
  ggsave(glue("11_ADT_demultiplex/plot/{sample_name}/ADT_log_counts_density.png"), width = 10, height = 7)
  
}

names(seurat_obj_ADT)

saveRDS(seurat_obj_ADT, "11_ADT_demultiplex/out/seurat_obj_ADT.rds")

# ---------------------------------------------------------------------------
# Define zero points for each sample in the sourced script
# ---------------------------------------------------------------------------

source("11_ADT_demultiplex/script/ADT_zero_points.R")
...
for (sample_name in sample_names_ADT){
  ...
  ggsave(
    glue("11_ADT_demultiplex/plot/{sample_name}/N_above_zero_point_{sample_name}.png"),
    width = 10, 
    height = 6
  )
  
  outdir_plot <- glue("11_ADT_demultiplex/plot/{sample_name}/individual_cell_annotation")
...
for (sample_name in sample_names_ADT) {
  ...
  ggsave(glue("11_ADT_demultiplex/plot/{sample_name}/barplot_ADT_class.png"), width = 10, height = 6)
  plot_barplot(data.frame(manual_ADT_ID = seurat_obj$manual_ADT_ID), "manual_ADT_ID")
  ggsave(glue("11_ADT_demultiplex/plot/{sample_name}/barplot_ADT_ID.png"), width = 10, height = 6)
  ...
  # Create heatmap
  pdf(glue("11_ADT_demultiplex/plot/{sample_name}/doublet_pattern_heatmap.pdf"), width = 8, height = 7, useDingbats = FALSE)
  ...
  outdir_umap <- glue("11_ADT_demultiplex/plot/{sample_name}/UMAP_plots")
  ...
  # ggsave(glue("11_ADT_demultiplex/plot/{sample_name}/demultiplexing/FeatureScatter.png"), width = 8, height = 6)
  ...
  ggsave(glue("11_ADT_demultiplex/plot/{sample_name}/RidgePlot.png"), width = 16, height = 20)
...
# Export xlsx file 
out_file <- "11_ADT_demultiplex/table/fol_freq.xlsx"
