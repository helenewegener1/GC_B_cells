...
# Make outdir
outdir <- glue("45_immcantation/plot/11_clone_mutations/{HH}")
...
clone <- df_heavy %>% 
  ...
  dplyr::slice(clone_nr) %>% pull(clone_subgroup_id_90_similarity)

# Look at clone
df_heavy %>% filter(clone_subgroup_id_90_similarity == clone) %>% dplyr::count(clone_subgroup_id_90_similarity, v_call_no_allele, j_call_no_allele, sort = TRUE)
