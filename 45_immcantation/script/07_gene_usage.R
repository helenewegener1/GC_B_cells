...
resolve_LC <- readRDS(glue("45_immcantation/out/rds/05_{HH}_resolve_LC.rds"))
...
outdir <- "45_immcantation/plot/07_gene_usage/"
...
df_heavy %>% filter(v_call_no_allele == this_gene) %>% dplyr::count(sample_clean, c_call, sort = T) %>% 
...
df_heavy %>% filter(v_call_no_allele == this_gene) %>% dplyr::count(sample_clean, L1_annotation, sort = T) %>% 
