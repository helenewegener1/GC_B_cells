...
for (HH in patients){
  ...
  for (clone_nr in n_clones_begin_list[[HH]]:n_clones){
    ...
    # Expand collapsed (identical-sequence) nodes back to individual sequence_id's
    idmap <- read_csv(idmap_file, col_names = c("node", "sequence_id"), show_col_types = FALSE) %>% 
      filter(node != "GL" & node %in% seq_nodes) %>% 
      separate_longer_delim(sequence_id, delim = ":")
    ...
    for (i in 1:length(clone_follicles)){
      ...
      # Make sure the sequences are in dist_mat
      # all(rownames(dist_mat) == colnames(dist_mat))
      clone_follicle_seq_ids <- clone_follicle_seq_ids[clone_follicle_seq_ids %in% rownames(dist_mat)]
      other_fols_seq_ids <- other_fols_seq_ids[other_fols_seq_ids %in% rownames(dist_mat)]
