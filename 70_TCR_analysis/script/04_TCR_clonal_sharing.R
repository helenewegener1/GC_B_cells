...
#   per compartment-shared clone. Spot-check `df_tcr$CTgene %>% head()`
...
# 
# df_tcr <- df_tcr %>% mutate(v_trb_call = extract_trbv(CTgene))
...
for (n_min_cells in c(1, 2)) {
  ...
  df_n_fols <- df_tcr %>%
    filter(sample_clean %in% SI_PP_sites) %>% 
    group_by(patient_id, CTstrict) %>%
    count(sample_clean_fol) %>%
    filter(n >= n_min_cells) %>%
    count(sample_clean_fol) %>%
    count(CTstrict, sort = TRUE) %>%
    dplyr::rename(n_fols = n) %>%
    ungroup()

  df_N_cells <- df_tcr %>%
    right_join(df_n_fols, by = c("patient_id", "CTstrict")) %>%
  ...
  df_N <- df_plot %>%
    select(-CTstrict) %>%
    count(patient_id, n_fols, mean_clone_size, median_clone_size, sort = TRUE)

  df_plot %>%
    ggplot(aes(x = patient_id, y = n_fols)) +
    geom_jitter(
    ...
    geom_text(
      data = df_N,
      aes(x = patient_id, y = n_fols, label = glue("{n} clones ({mean_clone_size}; {median_clone_size})")),
    ...
    labs(
      x = "Patient ID",
      y = "N follciles",
      title = glue("TCR clonal sharing across SI-PP follicles"),
      subtitle = glue("For each clone, how many follciles is it present in (at least {n_min_cells} cells)"),
  ...
  df_plot %>%
    mutate(n_fols_fct = n_fols %>% as.factor()) %>%
    ggplot(aes(x = patient_id, fill = n_fols_fct)) +
    ...
    labs(
      ...
      x = "Patient ID",
      y = "% of clones",
      title = glue("Percentage of clonal sharing across SI-PP follicles"),
      subtitle = glue("For each clone, how many follicles is it present in (at least {n_min_cells} cells)")
    )
...
#   ungroup()
# 
# build_trbv_pie_data <- function(df_tcr, df_shared_clones) {
# 
#   site_lookup <- df_tcr %>%
...
# }
# 
# dat_all <- build_trbv_pie_data(df_tcr, df_shared_clones)
# 
