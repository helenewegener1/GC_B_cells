...
# Define Ig genes
ig_genes <- grep(
  # "^IGH|^IGK|^IGL|^IGHV|^IGHD|^IGHJ|^IGKV|^IGKJ|^IGLV|^IGLJ", 
  # "^IGH|^IGK|^IGL|^IGHV|^IGHJ|^IGKV|^IGKJ|^IGLV|^IGLJ", # Gina: not including IGHD and IGHM, which are kind of hallmarks of naive B cells
  "^IGHV, ^IGLV, ^IGKV, ^IGLV",
...
# Scale data excluding Ig genes from variable features
variable_features <- VariableFeatures(seurat_merged)
variable_features_no_ig <- setdiff(variable_features, ig_genes)
VariableFeatures(seurat_merged) <- variable_features_no_ig

seurat_obj <- ScaleData(seurat_merged, features = variable_features_no_ig)
...
DimPlot(seurat_merged, reduction = "umap.unintegrated", group.by = "celltype_broad", split.by = "sample", label = TRUE, ncol = 3, label.size = 2) +
  NoLegend() +
  labs(title = "UMAP - RNA - pre integration", caption = glue("{n_dim}PCs")) +
  theme(
...
# REMOVE CONTAMINATION BEFORE INTEGRATING?
