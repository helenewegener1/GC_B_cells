...
def alignement_free_clone(repertoire,table_neg,W_l=150,per=0.1):
    ...
    table_r_f = pd.concat([df, df2], ignore_index=True)
    ...
    for i in range (0,max(clusters_cosine_full)):
        ...
        if temp_in[0].shape[0]>1:
            ...
            for j in range (0,combinations.__len__()):
                per_clone_score=per_clone_score+(table_clone_list[combinations[j][0]]==table_clone_list[combinations[j][1]])    
            sum_clustered_pairs=sum_clustered_pairs+per_clone_score
...
def truncate_sequence_v(table,W_l=200,E_l=0):
   ...
   # W_l=200
    ...
    for i in range (0,M):
        ...
        table_last = pd.concat([table_last, pd.DataFrame({'end': [table[i][s_l-W_l:s_l-E_l]]})], ignore_index=True)
    return table_last
...
def compute_cluster_stats(clusters_cosine_full,table,M):
    ...
    for i in range (0,max(clusters_cosine_full)):
        temp=np.where(clusters_cosine_full==i)
        if temp[0].shape[0]>1:
            junctionsizevec_cosine=np.append(junctionsizevec_cosine, np.unique(junction_length[temp]).shape[0])
    junctionsizevec_table=[]
    for i in range (0,int(max(table.CLONE[0:M_points]))):
        temp=np.where(table.CLONE[0:M_points]==i)
        if temp[0].shape[0]>1:
            junctionsizevec_table=np.append(junctionsizevec_table, np.unique(junction_length[temp[0]]).shape[0])
    return clustersizevec_cosine,clustersizevec_table,junctionsizevec_cosine,junctionsizevec_table
...
def vj_cell_extract(table,M):
    ...
    for i in range (0,table.__len__()):
        ...
        table_v = pd.concat([table_v, pd.DataFrame({'V': [table.V_CALL[i][0:15]]})], ignore_index=True)
        # table_j = table_j.append({'J': table.J_CALL[i][0:13]}, ignore_index=True)
        table_j = pd.concat([table_j, pd.DataFrame({'J': [table.J_CALL[i][0:13]]})], ignore_index=True)
    ...
    for i in range (0,M):
        ...
        table_vj = pd.concat([table_vj, pd.DataFrame({'VJ': [temp]})], ignore_index=True)
...
def awesome_cossim_top(A, B, ntop, lower_bound=0):
    ...
    C = A.dot(B)
    C = C.tocsr()
    # Zero out values below lower_bound
    C.data[C.data < lower_bound] = 0
    C.eliminate_zeros()
    return C
...
def truncate_sequence(table,W_l=200):
   ...
   # W_l=200
    ...
    for i in range (0,M):
        ...
        # table_last = table_last.append({'end': table.SEQUENCE[i][s_l-W_l:s_l-E_l]}, ignore_index=True)
        table_last = pd.concat([table_last, pd.DataFrame({'end': [table.SEQUENCE[i][s_l-W_l:s_l-E_l]]})], ignore_index=True)
    return table_last
...
def truncate_sequence_start(table,W_l=200):
   ...
   # W_l=200
    ...
    for i in range (0,M):
        s_l=table.SEQUENCE[i].__len__()
        # table_last = table_last.append({'end': table.SEQUENCE[i][E_l:E_l+W_l]}, ignore_index=True)
        table_last = pd.concat([table_last, pd.DataFrame({'end': [table.SEQUENCE[i][E_l:E_l+W_l]]})], ignore_index=True)
    return table_last
...
def truncate_sequence_s(table,W_l=200,E_l=0):
   ...
   # W_l=200
    ...
    for i in range (0,M):
        s_l=table.SEQUENCE[i].__len__()
        # table_last = table_last.append({'end': table.SEQUENCE[i][s_l-W_l-E_l:s_l-E_l]}, ignore_index=True)
        table_last = pd.concat([table_last, pd.DataFrame({'end': [table.SEQUENCE[i][s_l-W_l-E_l:s_l-E_l]]})], ignore_index=True)
    return table_last
...
def truncate_sequence_r(table,W_l=200,E_l=0):
   ...
   # W_l=200
    ...
    for i in range (0,M):
        ...
        # table_last = table_last.append({'end': table.SEQUENCE_INPUT[i][s_l-W_l-E_l:s_l-E_l]}, ignore_index=True)
        table_last = pd.concat([table_last, pd.DataFrame({'end': [table.SEQUENCE_INPUT[i][s_l-W_l-E_l:s_l-E_l]]})], ignore_index=True)
    return table_last
...
def truncate_sequence_rs(table,W_l=200):
   ...
   # W_l=200
    ...
    for i in range (0,M):
        ...
        table_last = pd.concat([table_last, pd.DataFrame({'end': [table.values[i][:W_l]]})], ignore_index=True)
    return table_last
...
def truncate_sequence_rf(table,W_l=200,E_l=0):
   ...
   # W_l=200
    ...
    for i in range (0,M):
        s_l=table.values[i].__len__()
        # table_last = table_last.append({'end': table.values[i][s_l-W_l-E_l:s_l-E_l]}, ignore_index=True)
        table_last = pd.concat([table_last, pd.DataFrame({'end': [table.values[i][s_l-W_l-E_l:s_l-E_l]]})], ignore_index=True)
    return table_last
...
def sensitivity_comp(table_clone_list,clusters_cosine_full):
    ...
    for i in range (0,max(table_clone_list)):
        ...
        if temp_in[0].shape[0]>1:
            ...
            per_clone_score=0
            for j in range (0,combinations.__len__()):
                per_clone_score=per_clone_score+(clusters_cosine_full[combinations[j][0]]==clusters_cosine_full[combinations[j][1]])    
            sum_clustered_pairs=sum_clustered_pairs+per_clone_score
...
def truncate_sequence_start_r(table,W_l=200):
   ...
   # W_l=200
    ...
    for i in range (0,M):
        s_l=table.SEQUENCE_INPUT[i].__len__()
        # table_last = table_last.append({'end': table.SEQUENCE_INPUT[i][E_l:E_l+W_l]}, ignore_index=True)
        table_last = pd.concat([table_last, pd.DataFrame({'end': [table.SEQUENCE_INPUT[i][E_l:E_l+W_l]]})], ignore_index=True)
    return table_last
    ...
    for i in range(0,M):
        ...
        if (matches_clust_ind[0].shape[0]>1):
            ...
            if temp.shape[0]==0:
                ...
                clusters_cosine_full[matches_clust_ind[0]]=clusters_cosine_full[matches_clust_ind[0][temp[0]]]
    return clusters_cosine_full

def spec_vs_thresh_eval(dist2nearestcosine,dist2nearestcosine_neg_a,dist2nearestcosine_neg_b,labels_list):
    spec_arr_var_a=[]
    spec_arr_var_b=[]
