...
compare %>% 
  ...
  labs(
    ...
    title = glue("{HH}: Top 10 LC VS Alignment free clones"),
...
compare %>% 
  ...
  labs(
    ...
    y     = "LC clones",
    title = glue("{HH}: Top 10 Alignment free VS LC clones"),
    caption = glue("Minimum count: {min_count}")
  ) +
  theme_minimal(base_size = 10) + 
  theme(
...
# ------------------------------------------------------------------------------
# Visualize - LC subclones (heavy and light)
# ------------------------------------------------------------------------------

outdir <- glue("47_alignment_free_clone_identification/plot/LC_subclone_vs_align_free")
dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

# Clean dfs
LC_clean <- LC %>% filter(locus == "IGH") %>% select(cell_id, clone_subgroup_id)
align_free_clean <- align_free %>%  select(ID, CLONE)

# Define top clones
top_clones_names <- list(
  "LC" = LC_clean %>% count(clone_subgroup_id, sort = TRUE) %>% head(10) %>% pull(clone_subgroup_id),
  "align_free" = align_free_clean %>% count(CLONE, sort = TRUE) %>% head(10) %>% pull(CLONE) %>% as.character()
)

top_clones_count <- list(
  "LC" = LC_clean %>% count(clone_subgroup_id, sort = TRUE) %>% head(10) %>% pull(n),
  "align_free" = align_free_clean %>% count(CLONE, sort = TRUE) %>% head(10) %>% pull(n) 
)
...
# Make table
compare <- table(LC_clean$clone_subgroup_id, align_free_clean$CLONE) %>% 
  as.data.frame() %>%
  setNames(c("LC_clones", "align_free_clones", "Count")) 
...
compare %>% 
  ...
  labs(
    x     = "LC clones",
    y     = "Alignment free clones",
    title = glue("{HH}: Top 10 LC (sub) VS Alignment free clones"),
    caption = glue("Minimum count: {min_count}")
  ) +
  theme_minimal(base_size = 10) + 
  theme(
...
ggsave(glue("{outdir}/{HH}_LCsub_VS_align_free_clones.png"), width = 10, height = 12)
...
compare %>% 
  ...
  labs(
    x     = "Alignment free clones",
    y     = "LC clones",
    title = glue("{HH}: Top 10 Alignment free VS LC (sub) clones"),
    caption = glue("Minimum count: {min_count}")
  ) +
  theme_minimal(base_size = 10) + 
  theme(
...
ggsave(glue("{outdir}/{HH}_align_free_VS_LCsub_clones.png"), width = 10, height = 12)
