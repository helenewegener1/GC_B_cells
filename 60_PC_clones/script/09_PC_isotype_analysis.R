...
lapply(patients, function(HH){
  ...
  df_overlap_plot %>% 
    ...
    labs(
      ...
      title = glue("{HH}: Clonal overlap between isotypes in LP PCs"),
  ...
  ggsave(glue("{outdir}/{HH}_pc_isotype_overlap_N.png"))
...
lapply(patients, function(HH){
  ...
  ggsave(glue("{outdir}/{HH}_pc_isotype_overlap_pct_smaller_set.png"))
...
lapply(patients, function(HH){
  ...
  df_overlap_plot %>% 
    ...
    labs(
      ...
      title = glue("{HH}: Clonal overlap between PC isotypes in LP"),
  ...
  ggsave(glue("{outdir}/{HH}_pc_isotype_overlap_MHI.png"))
...
lapply(patients, function(HH){
  ...
  df_plot %>% 
    ...
    scale_y_break(c(max_ys[2]+5, max_ys[1]-100), scales = 1/5) +
    scale_y_continuous(
...
lapply(patients, function(HH){
  ...
  df_iga <- df_both %>% 
    filter(patient_id == HH & c_call_grouped %in% c("IGHA1", "IGHA2")) %>% 
    mutate(condition = sample_clean %>% str_remove(glue("{HH}-")))
  ...
  df_plot <- df_iga %>% 
    group_by(clone_subgroup_id_90_similarity) %>% 
    summarise(
      n_igha1 = sum(c_call_grouped == "IGHA1"),
      n_igha2 = sum(c_call_grouped == "IGHA2"),
      .groups = "drop"
    ) %>% 
    mutate(
      n_cells_total = n_igha1 + n_igha2,
      IGA1_percentage = (n_igha1 / n_cells_total) * 100
    ) %>% 
    count(IGA1_percentage)
  
  max_ys <- sort(df_plot$n, decreasing = TRUE)[1:3]
  ...
  df_plot %>% 
    ...
    scale_y_break(c(max_ys[2], max_ys[1] - 100), scales = 1/5) +
    scale_y_break(c(max_ys[3], max_ys[2] - 100), scales = 1/5) +
    scale_x_continuous(
      breaks = scales::breaks_width(10)
    ) +
    theme_bw() + 
    theme(axis.title.x.top = element_text(), axis.text.x.top = element_text()) +
    labs(
      title = glue("{HH}: IgA clones - ratio of IgA1 to IgA2"), 
      x = "% IgA1/(IgA1+IgA2)", 
      y = "N clones", 
      caption = "x = 0 --> 100% IgA2"
    )
