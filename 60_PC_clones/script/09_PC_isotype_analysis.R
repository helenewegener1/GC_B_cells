...
# Prep output
outdir = glue("60_PC_clones/plot/09_isotype_analysis/")
...
for (n_min_cells in c(1, 2)){
  ...
  df_n_isotypes <- df_both %>% 
    count(patient_id, clone_subgroup_id_90_similarity, c_call) %>% 
  ...
  df_n_isotypes %>% 
    ...
    labs(
      title = "N isotypes per clone per patient",
  ...
  df_n_isotypes %>% 
    ...
    scale_fill_viridis_d(option = "magma", direction = -1, na.value = "grey90") +  
    labs(
      title = "N isotypes per clone per sample_clean",
      subtitle = glue("For each clone, how many isotypes are found (at least {n_min_cells} cells)"),
      x = "Sample", 
      y = "N clones", 
      fill = "N isotypes"
...
clone_isotype_counts <- df_both %>% 
  filter(!is.na(c_call)) %>% 
  count(patient_id, c_call, clone_subgroup_id_90_similarity, name = "n_cells") %>% 
...
for (HH in unique(clone_isotype_counts$patient_id)) {
  ...
  isotypes_HH <- unique(df_HH$c_call)
  ...
  for (pair in isotype_pairs) {
    ...
    df_a <- df_HH %>% filter(c_call == iso_a)
    df_b <- df_HH %>% filter(c_call == iso_b)
...
df_overlap_plot %>% 
  ...
  labs(
    ...
    title = "Clonal overlap between isotypes in LP PCs",
...
ggsave(glue("{outdir}/pc_isotype_overlap_N.png"), width = 13, height = 7)
...
ggsave(glue("{outdir}/pc_isotype_overlap_pct_smaller_set.png"), width = 13, height = 7)
...
df_overlap_plot %>% 
  ...
  labs(
    ...
    title = "Clonal overlap between PC isotypes in LP",
...
ggsave(glue("{outdir}/pc_isotype_overlap_MHI.png"), width = 13, height = 7)
...
#   select(c_call, q, d, d_ci_lower = d_lower, d_ci_upper = d_upper)
...
clone_sizes <- df_both %>% 
  filter(!is.na(c_call)) %>% 
  count(patient_id, c_call, clone_subgroup_id_90_similarity, name = "clone_size")

clone_sizes %>% 
  ggplot(aes(x = c_call, y = clone_size, fill = c_call)) + 
  ...
  labs(
    ...
    title = "Clone size by isotype in LP PC clones",
    subtitle = "Each point is one clone."
  ) + 
  theme_bw() + 
  theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1))

ggsave(glue("{outdir}/pc_isotype_clone_size.png"), width = 10, height = 6)

# Kruskal-Wallis test per patient (non-parametric, since clone sizes are heavily right-skewed)
clone_sizes %>% 
  group_by(patient_id) %>% 
  summarise(kruskal_p = kruskal.test(clone_size ~ c_call)$p.value)
...
lapply(patients, function(HH){
  ...
  clone_condition_counts <- df_condition %>% 
    count(patient_id, c_call, condition, clone_subgroup_id_90_similarity, name = "n_cells") %>% 
    filter(n_cells >= n_min_cells)
  
  conditions_HH <- unique(clone_condition_counts$condition)
  isotypes_HH <- unique(clone_condition_counts$c_call)
  ...
  for (iso in isotypes_HH) {
    ...
    df_iso <- clone_condition_counts %>% filter(c_call == iso)
...
lapply(patients, function(HH){
  ...
  df_iga <- df_both %>% 
    filter(
      patient_id == HH & c_call %in% c("IGHA1", "IGHA2")
    ) %>% 
    mutate(
      condition = sample_clean %>% str_remove(glue("{HH}-"))
    )
  ...
  df_plot <- df_iga %>% 
    ...
    count(c_call, name = "n_cells") %>% 
    mutate(
      n_cells_total = sum(n_cells),
      IGA1_percentage = ifelse(c_call == "IGHA1", (n_cells/n_cells_total) * 100, NA)
      # IGA2_percentage = ifelse(c_call == "IGHA2", (n_cells/n_cells_total) * 100, NA)
    ) %>% 
    ungroup() %>% 
