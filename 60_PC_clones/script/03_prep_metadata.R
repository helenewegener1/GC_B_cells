...
fasta_path <- glue("60_PC_clones/fasta")
...
LP_sites <- lapply(patients, function(HH) grep("LP", unique(df_both[[HH]]$sample_clean_fol), value = TRUE)) %>% unlist()
...
for (site in LP_sites){
  ...
  for (clone_nr in clone_nrs){
    ...
    # clone_nr <- 1
    filename <- grep(glue("{site}_clone_nr_{clone_nr}_"), fasta_files, value = TRUE)
    ...
    write.csv(gctree_meta, glue("60_PC_clones/gctree_meta/{sample}_gctree_meta.txt"), row.names = FALSE, quote = FALSE)
    ...
    write.csv(L1_counts, glue("60_PC_clones/gctree_meta/{sample}_L1_counts.csv"), row.names = FALSE)
    ...
    write.csv(sample_clean_fol_counts, glue("60_PC_clones/gctree_meta/{sample}_sample_clean_fol_counts.csv"), row.names = FALSE)
