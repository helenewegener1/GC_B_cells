...
# Look at junction lengths
range(subset_clones_vj$junction_length)
hist(subset_clones_vj$junction_length)

# Extract sequences
seqs <- subset_clones_vj$junction
seq_names <- subset_clones_vj$sequence_id

# Get metadata of sequences
seqs_meta <- subset_clones_vj %>% 
  mutate(
    label = sequence_id, 
