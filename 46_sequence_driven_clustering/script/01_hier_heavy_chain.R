...
# Focus on heavy chain
spec_clones_vj_HH_heavy <- spec_clones_vj_HH %>% filter(locus == "IGH")

# N J genes
spec_clones_vj_HH_heavy %>% count(j_call, sort = TRUE)
spec_clones_vj_HH_heavy$j_call %>% str_split_i("\\*", 1) %>% table() %>% sort()
...
seqs_meta <- spec_clones_vj_HH_heavy %>% 
  filter(sequence_id %in% seq_names) %>% 
...
lengths <- nchar(seqs_meta$sequence)
...
top_clones <- spec_clones_vj_HH_heavy %>% count(clone_id, sort = TRUE) %>% head(5) %>% pull(clone_id)
top_clones
