...
df_plot %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(
    title = "Frequency of memory B cell clone sizes",
...
df_plot %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(
    title = "Isotypes of memory B cell clones",
    x = "Samples", 
    y = "N cells", 
    fill = "Isotype"
  )
...
df_plot %>% 
  ggplot(aes(x = sample_clean, y = n)) + 
  geom_col(aes(fill = c_call_grouped), position = "fill") + 
  scale_fill_manual(values = isotype_grouped_colors_custom) + 
  scale_y_continuous(labels = scales::percent) + 
  geom_text(
    data = df_plot %>% group_by(sample_clean) %>% summarise(total_count = sum(n)),
    mapping = aes(x = sample_clean, y = 1.025, label = total_count),
    size = 3
  ) +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(
    ...
    y = "% of cells", 
    fill = "Isotype", 
    caption = "Number of top of bar is total memory B cell count"
  )

ggsave(glue("{outdir6}/isotype_barplot_percentage.png"))
...
lapply(patients, function(HH){
  ...
  # Subset seurat object to memory B cells 
  seurat_integrated_mem <- subset(seurat_integrated, patient == HH & L1_annotation == "Memory_Bcells")
  df_HH <- df_both %>% filter(patient_id == HH)
  
  rownames(seurat_integrated_mem[[]]) %>% head()
  df_HH$cell_id %>% head()
  
  # Update cell_id
  seurat_integrated_mem[[]] <- seurat_integrated_mem[[]] %>% 
    mutate(
      cell_id = glue("{sample}_{rownames(.)}") %>% str_remove("_\\d")
    )
  
  seurat_integrated_mem$cell_id %>% head()
  df_HH$cell_id %>% head()
  
  table(seurat_integrated_mem$cell_id %in% df_HH$cell_id)
  
  # Subset to cells in df_both 
  seurat_integrated_mem <- subset(seurat_integrated_mem, cell_id %in% df_HH$cell_id)
  
  table(seurat_integrated_mem$cell_id %in% df_HH$cell_id)
  
  # Add clone information to seurat object 
  seurat_integrated_mem[[]] <- seurat_integrated_mem[[]] %>% left_join(df_HH, by = "cell_id")
  
  # APackOfTheClones
  Idents(seurat_integrated_mem) <- "sample"
  vizAPOTC(
    seurat_integrated_mem,
    ...
    plot_annotation(title = glue("{HH}: Memory B cell clone sizes"))
...
lapply(patients, function(HH){
  ...
  ggplot() + 
    ...
    coord_equal() + 
    labs(
      title = glue("{HH}: Memory B cell clones"), 
