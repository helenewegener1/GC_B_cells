...
outdir_combined <- glue("61_memory_clones/plot/07_diversity_analysis/combined{extra}")
...
plot(curve, legend_title="Sample - Memory B cells") + theme_minimal()
...
plot(sample_curve, main_title="Sample diversity - Memory B cells", 
...
for (min_n in c(20, 30, 100)){
  ...
  ggplot(shannon_vals, aes(x = sample_clean, y = d)) +
    ...
    labs(
      ...
      title = glue("{HH} - Memory B cells: Shannon diversity per compartment"),
...
ggplot(d50_per_follicle_long, aes(x = sample_clean, y = value, fill = metric)) +
  ...
  labs(
    ...
    y = "Number of clones",
    fill = NULL,
    title = glue("{HH} - Memory B cells: Clonal dominance per compartment"),
    caption = "Numbers above bars indicate total memory B cell clone count per compartment"
  ) 

ggsave(glue("{outdir}/clonal_D20_D50_plot.png"), width = 10, height = 8)
...
ggplot(gini_per_follicle, 
  ...
  labs(
    x = "Compartment",
    y = "Gini coefficient",
    title = glue("{HH} - Memory B cells: Gini coefficient per compartment")
  ) 

ggsave(glue("{outdir}/gini_coef.png"), width = 12, height = 8)
...
ggplot(gini_combined, aes(x = sample_clean, y = gini, color = patient)) +
  ...
  labs(
    x = "Sample",
    y = "Gini coefficient",
    title = "Gini coefficient per compartment - Memory B cells",
    subtitle = "Each point represents one compartment"
  ) #+
   # theme(legend.position = "none")

ggsave(glue("{outdir_combined}/gini_coef_combined.png"), width = 10, height = 8)
...
for (min_n in c(30, 100)) {
  ...
  ggplot(shannon_combined, aes(x = sample_clean, y = d, color = patient)) +
    ...
    labs(
      x = "Sample",
      y = "Shannon diversity (q=1)",
      title = glue("Shannon diversity per compartment - Memory B cells"),
      subtitle = glue("Compartments with <{min_n} memory B cells excluded."),
      caption = "Bootstrapped estimates (100 resamples, 95% CI)"
    ) +
    theme(legend.position = "none")
  
  ggsave(glue("{outdir_combined}/shannon_diversity_min_n_{min_n}_combined.png"), width = 10, height = 8)
