...
dir.create(glue("61_memory_clones/gctree_meta"), recursive = TRUE)

patients <- names(df_both)

fasta_path <- glue("61_memory_clones/fasta")
...
for (HH in patients){
  ...
  for (clone_nr in clone_nrs){
    ...
    tryCatch({
      idmap <- read.csv(glue("61_memory_clones/out/{sample}/idmap.txt"), header = FALSE, col.names = c("seq_unique", "seq_name")) 
    }, error = function(e) {
      message("Skipping ", HH, " clone nr ", clone_nr, " - ", conditionMessage(e))
    })
    ...
    write.csv(gctree_meta, glue("61_memory_clones/gctree_meta/{sample}_gctree_meta.txt"), row.names = FALSE, quote = FALSE)
    ...
    write.csv(L1_counts, glue("61_memory_clones/gctree_meta/{sample}_L1_counts.csv"), row.names = FALSE)
    ...
    write.csv(isotype_counts, glue("61_memory_clones/gctree_meta/{sample}_isotype_counts.csv"), row.names = FALSE)
    ...
    write.csv(sample_clean_fol_counts, glue("61_memory_clones/gctree_meta/{sample}_sample_clean_fol_counts.csv"), row.names = FALSE)
