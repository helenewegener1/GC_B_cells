...
# Prep output
outdir = glue("61_memory_clones/plot/08_clonal_sharing/")
...
for (n_min_cells in c(1, 2)){
  ...
  df_plot %>% 
    ...
    labs(
      x = "Patient ID",
      y = "N compartments",
      title = "Clonal sharing across compartments - Memory B cells",
  ...
  df_plot %>% 
    ...
    labs(
      ...
      x = "Patient ID",
      y = "% of clones",
      title = "Percentage of clonal sharing across compartments - Memory B cells",
      subtitle = glue("For each clone, how many compartments is it present in (at least {n_min_cells} cells)")
    )
